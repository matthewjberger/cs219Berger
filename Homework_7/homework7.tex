...
\begin{homeworkProblem}[12]
	\begin{itemize}
		\item[12.6)] Compare zero-, one-, two-, and three-address machines by writing programs to compute $X=\frac{(A+BXC)}{(D-EXF)}$ for each of the four machines. The instructions available for use are as follows:
		...
		\begin{itemize}
			\item SUBS X = Subtract the contents of location X from the accumulator, and store the result in location X and the accumulator.
			\item JUMP X = Place address X in the program counter.		
		\end{itemize}
